...
export default createRule("comma-style", {
  ...
  create(context) {
    ...
    const exceptions = {} as Record<
      NodeType["type"] | "ArrayExpression" | "ObjectExpression",
      boolean
    >;
    ...
    function getFixerFunction(
      styleType: string,
      previousItemToken: TOMLToken,
      commaToken: TOMLToken,
      currentItemToken: AST.Token,
    ) {
      const text =
    ...
    function validateComma<T extends NodeType, const K extends keyof T>(
      ...
      const items = node[property] as (AST.Token | AST.TOMLNode)[];
      const arrayLiteral = node.type === "TOMLArray";

      if (items.length > 1 || arrayLiteral) {
        // seed as opening [
        let previousItemToken: TOMLToken = sourceCode.getFirstToken(node);
