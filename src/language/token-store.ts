import type { TOMLSyntaxElement, TOMLToken } from "./toml-source-code";
import type { AST } from "toml-eslint-parser";

/**
 * Binary search for the index of the first token that is after the given location.
 */
function search(tokens: TOMLToken[], location: number): number {
  let minIndex = 0;
 ...
 */
function isComment(token: TOMLToken): token is AST.Comment {
 ...
 */
function isNotComment(token: TOMLToken): boolean {
...
export class TokenStore {
   ...
   */
  private readonly allTokens: TOMLToken[];
  ...
  public constructor(params: { ast: AST.TOMLProgram }) {
...
export type FilterPredicate = (tokenOrComment: TOMLToken) => boolean;
