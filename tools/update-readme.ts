...
let insertText = `\n${renderRulesTableContent(
  2,
  (name) => `https://ota-meshi.github.io/eslint-plugin-toml/rules/${name}.html`,
)}\n`;
if (isWin) {
...
fs.writeFileSync(
  ...
  newReadme
    ...
    .replace(
      /\[LICENSE\]\(LICENSE\)/gu,
      "[LICENSE](https://github.com/ota-meshi/eslint-plugin-toml/blob/main/LICENSE)",
    )
    .replace(/<!--DOCS_IGNORE_START-->[\s\S]*?<!--DOCS_IGNORE_END-->/gu, "")
    .replace(/\(https:\/\/ota-meshi.github.io\/eslint-plugin-toml/gu, "(.")
    .replace(/\n{3,}/gu, "\n\n"),
  "utf8",
);
...
fs.writeFileSync(
  ...
  docsUserGuide
    ...
    .replace(
      /\(https:\/\/ota-meshi.github.io\/eslint-plugin-toml(.*?)\)/gu,
